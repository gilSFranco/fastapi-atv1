...
def testando_listar_estudante_por_id_sucesso():
    ...
    assert response.status_code == 200
    assert response.json() == {"id": 1, "nome": "Gilmar", "email": "gilmar@gmail.com"}
...
def testando_criar_estudante_sucesso():
    ...
    assert response.json() == {"id": 2, "nome": "Victor", "email": "victor@gmail.com"}


def testando_criar_estudante_falha():
    response = client.post(
        "/estudantes", json={"email": "lucas@gmail.com"}
    )
